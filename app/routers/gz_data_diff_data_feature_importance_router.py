...
service: GZDataDiffDataFeatureImportanceService = None

def get_service() -> GZDataDiffDataFeatureImportanceService:
    if service is None:
        raise RuntimeError("GZDataDiffDataFeatureImportanceService not initialized")
    return service

@router.get(
    "/GET_GZ_data_diff_data_feature_importance",
    summary="查詢 GreenZone 的差異分析資料的前十二大重要特徵",
    description="透過條件查詢 GreenZone 的差異分析資料的前十二大重要特徵"
)
def get_gz_data_diff_data_feature_importance(
    ...
    timetag: str = Query(None, alias="qdate", description="格式 歸屬日期 yyyy-mm-dd"),
    bdate: str = Query(None, alias="bdate", description="格式 基準時間 yyyy-mm-dd"),
    cdate: str = Query(None, alias="cdate", description="格式 對照時間 yyyy-mm-dd"),
    wmax: str = Query(None, alias="wmax", description="格式 基重最大值 如61"),
    wmin: str = Query(None, alias="wmin", description="格式 基重最小值 如55"),
    svc: GZDataDiffDataFeatureImportanceService = Depends(get_service)
):
    result = svc.get_feature_importance(variable_name, machine, ptype, smax, smin, timetag, bdate, cdate, wmax, wmin)
    return JSONResponse(content=result)


@router.post("/GET_GZ_data_diff_data_feature_importance")
def post_gz_data_diff_data_feature_importance():
