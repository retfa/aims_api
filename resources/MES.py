...
class Defect_induced_recycle_analysis_report:
    ...
    def fetch(self, stime: str, etime: str, mname: str): 
        ...
        if not df_all_data.empty:
            ...
            if dt.month == '12':
                year, month = dt.year+1, dt.month+1
            else:
        ...
        while (year < end_year) or (year == end_year and month <= end_month):
            ...
            if mname =='21':
                try:
                    if ym_str in ['202503','202504','202505','202506','202507','202508','202509','202510']:
...
#                     print(f"機台{mname} {ym_str} 匯入失敗: {e}") 
                ...
                try:
                    if ym_str in ['202406','202407','202408','202409','2024010','202411','202503','202504','202505','202506','202507','202508']:
                        file_path = fr'\\Srvafp1\週報\生產三處\各段加工得率及回爐率\AOI瑕疵檢測AI自動辨識系統統計資料\{ym_str}加工得率與不良品改善統計表-生產三處.xlsx'
                    else:                 
                        file_path = fr'\\Srvafp1\週報\生產三處\各段加工得率及回爐率\{ym_str}加工得率與不良品改善統計表-生產三處.xlsx'
            ...
            with srv_SRVAD1['create_engine'][0].connect() as conn:
             ...
             (
                    select 
                        ...
                        CASE WHEN ptype IN ('KL00','SL00') THEN '格拉新' ELSE '1' END AS IS_KL
                    from (
              ...
              SELECT t.bdate,t.mname,t.ptype,t.tl,t.pht,
                  CASE WHEN (SELECT COUNT(*) FROM raw_data WHERE ptype IN ('SL00','KL00')) = 1 and t.IS_KL = '格拉新' AND n.weigh>ISNULL(m.weigh,0) THEN n.weigh ELSE m.weigh END as s5,
                  t.s6
                FROM raw_data t
                LEFT JOIN (
        ...
        def find_W_Summary(stime, etime, df_Ampaper_category):
            ...
            while current_date <= end_date:
                ...
                if not df_result_W_quality_reason.empty:
                    ...
                    df_result_W_quality_reason['reason_str'] = df_result_W_quality_reason['dest'] + df_result_W_quality_reason['weigh'].round(3).astype(str) + 'T'
                    df_result_W_quality_reason = df_result_W_quality_reason.groupby(['bdate','class'])['reason_str'].apply(''.join).reset_index()
                    df_result_W_quality_reason.columns = ['bdate', 'class', 'df_result_W_reason_str']
